...
export class AdminRacesManagementComponent implements OnInit {
  ...
  constructor(private _grandPrixService:GrandPrixService,
    ...
    private _resultService:ResultService,private _driverService:DriverService) {
   ...
   continuarMundial(){
    ...
    this._ligaService.updateCurrentRound(2,nextRound).subscribe(
      () => {
  ...
  ngOnInit(): void {
    ...
    this._ligaService.getLiga(2).subscribe(apiLiga=>{
      ...
      this._grandPrixService.getGrandPrixByRound(this.liga?.currentRound).subscribe(apiGrandPrix=>{
